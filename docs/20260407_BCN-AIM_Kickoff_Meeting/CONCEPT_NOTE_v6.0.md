...
**Proposed Role for BCN-AIM Lab / fedder.ai:** BCN-AIM Lab (University of Barcelona) joins as the **EU Co-Principal Investigator and Technical Lead for the FL Architecture**, with a key role in validating the approach against European Health Data Space (EHDS) standards. fedder.ai is envisioned as EU technical partner for scalable federated learning deployment.

**MVP & Timeline:** 12-month pilot predicting hospitalization risk for Type 2 Diabetes patients across 3–5 Georgian clinics. Target: AUC >0.75, zero raw data export (audited). Total budget: €2,275,000 (EIC Pathfinder ceiling: €4M).

**One-Sentence Pitch:** We unite medical data from clinics and pharmaceutical companies to train AI without transferring raw patient data, with measurable contributions from each participant and transparent benefit sharing.
...
## 4. Contribution Measurement — Federated Shapley Value
...
**Minimal Dataset:** Age (5-yr bins), sex, T2DM diagnosis year, HbA1c (last), BMI, complications (binary flags), hospitalization in past 12 months, hospitalization in next 12 months (target variable).
...
## 10. Consortium Structure (Proposed)
...
| **BCN-AIM Lab (Univ. Barcelona)** | **Spain (EU)** | **EU Co-PI, FL Architecture Lead, EHDS Validation** | **To be confirmed** |
| **fedder.ai** | **EU** | **EU Technical Partner — scalable FL deployment** | **To be confirmed** |
