...
## [2026-04-05] FCLC ↔ CDATA: Федеративная калибровка модели старения
**Source:** Перекрёстный анализ FCLC и CDATA в свете новых статей
**Status:** [ ] proposed

**Суть:** CDATA v3.4 (R²=0.84) откалиброван на 5 датасетах. Для Aging Cell нужна валидация на ≥18 датасетах. FCLC — платформа, которая может это дать без передачи данных пациентов.

**Предлагаемая интеграция:**
- Каждый клинический узел FCLC хранит данные о стволовых клетках (CHIP VAF, эпигенетический возраст, фрейлти индекс)
- Локально обучает CDATA-модуль на своих данных → отправляет только градиенты параметров (τ_protection, π₀)
- Оркестратор усредняет → CDATA v3.5 с N-центровой калибровкой
- Результат: первая многоцентровая валидация CDATA → публикация в Nature Aging

**Технически:** добавить `CdataAdapter` в fclc-node — читает OMOP `measurement` domain (CHIP VAF, frailty scores) → форматирует для cell_dt_validation модуля.

---

## [2026-04-05] Ze-теория ↔ Дифференциальная приватность: формальное соответствие
**Source:** Ze_and_Entropy.md (CDATA/Articles) + FCLC DP архитектура
**Status:** [ ] proposed

**Суть:** Ze-бюджет τ_Z и DP-бюджет ε формально аналогичны:

| Ze-теория | Дифференциальная приватность |
|-----------|------------------------------|
| T-событие (сюрприз) → τ_Z уменьшается | Запрос к данным → ε уменьшается |
| τ_Z = 0 → Ze-наблюдатель умирает | ε_total исчерпан → данные закрываются |
| Аксиома Z2: τ_Z монотонно убывает | Бюджет DP монотонно убывает |
| Ze-энтропия S_Ze = k ln N_Ze | Информационная утечка ≤ exp(ε) |

**Следствие:** Выбор ε=2.0 per round можно обосновать из Ze-аксиом: это «стоимость одного T-события» для узла-наблюдателя. Рецензенты JMIR оценят теоретическое обоснование DP параметров.

**Где зафиксировать:** добавить раздел «Theoretical Foundations» в JMIR статью, ссылаясь на 5+_Ze_Foundations_of_Physics.md.
